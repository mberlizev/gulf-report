...
def update_deadline_banner(html):
    """Update the deadline countdown (April 6 Trump deadline)."""
    ...
    log.info("Updated deadline: %d days left", max(days_left, 0))
    return html


def update_trend_date(html, date_str):
    """Update the trend block date header."""
    dt = datetime.strptime(date_str, "%Y-%m-%d")
    date_ru = "%d %s %d" % (dt.day, MONTHS_RU_FULL[dt.month], dt.year)
    pattern = r'(<div class="trend-t">\u0422\u0440\u0435\u043d\u0434\u044b \u00b7 )[^<]*(</div>)'
    html = re.sub(pattern, r'\g<1>%s\2' % date_ru, html)
...
def main():
    ...
    # Update deadline banner countdown
